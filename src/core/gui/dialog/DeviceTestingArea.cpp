...
static void saveLog(const std::stringstream& fullLog, const std::stringstream& ellipsizedLog,
    ...
    auto callback = [fullLog = fullLog.str(), ellipsizedLog = ellipsizedLog.str(), gdkeventslog = gdkeventslog.str(),
        ...
        if (p && !p->empty()) {
            int errorCode = 0;
            auto* zip = zip_open(p->c_str(), ZIP_CREATE | ZIP_TRUNCATE, &errorCode);
            if (!zip) {
                // TODO handle error
            ...
            addSource("settings.xml", zip_source_file(zip, settings.c_str(), 0, ZIP_LENGTH_TO_END));
#else
            addSource("settings.xml", zip_source_file(zip, settings.c_str(), 0, -1));
#endif
